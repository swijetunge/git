...
if(isset($_GET['convert'])){
	...
	if($_POST['textfield'] == "" || $_POST['textfield2'] == "" || $_POST['textfield4'] == ""){
		...
		$Class_mysql_to_php = new MySQL_to_SP($_POST['textfield'], $_POST['textfield2'], $_POST['textfield3'], $_POST['textfield4'], $_POST['textfield5']);
	
		if(isset($_POST['checkbox'])) $Class_mysql_to_php->setConstruct(true); else $Class_mysql_to_php->setConstruct(false);
		if(isset($_POST['checkbox2'])) $Class_mysql_to_php->setLoad_row_from_key(true); else $Class_mysql_to_php->setLoad_row_from_key(false);
		if(isset($_POST['checkbox3'])) $Class_mysql_to_php->setDelete_row_from_key(true); else $Class_mysql_to_php->setDelete_row_from_key(false);
		if(isset($_POST['checkbox4'])) $Class_mysql_to_php->setSave_active_row(true); else $Class_mysql_to_php->setSave_active_row(false);
		if(isset($_POST['checkbox5'])) $Class_mysql_to_php->setSave_active_row_as_new(true); else $Class_mysql_to_php->setSave_active_row_as_new(false);
		if(isset($_POST['checkbox6'])) $Class_mysql_to_php->setOrder_Keys(true); else $Class_mysql_to_php->setOrder_Keys(false);
		if(isset($_POST['checkbox7'])) $Class_mysql_to_php->setGeters(true); else $Class_mysql_to_php->setGeters(false);
		if(isset($_POST['checkbox8'])) $Class_mysql_to_php->setSeters(true); else $Class_mysql_to_php->setSeters(false);	
		if(isset($_POST['checkbox9'])) $Class_mysql_to_php->setProcs(true); else $Class_mysql_to_php->setProcs(false);
		
		$Class_mysql_to_php->CreateClassFiles();
...
<form id="form1" name="form1" method="post" action="?convert">
  <table width="741" border="0" align="center">
    <tr>
      <td height="30" colspan="3" bgcolor="#EEEEEE" class="title">Convert: MySQL Table -&gt; PDO class</td>
    </tr>
    <tr>
    ...
    <tr>
      <td class="style8"><div align="right">Stored Procedures</div></td>
      <td colspan="2" class="style9"><label>
        <input name="checkbox9" type="checkbox" value="true" checked="checked" />
      </label>        <label>create Stored Procedures</label></td>
    </tr>
    <tr>
      <td class="style8">&nbsp;</td>
      <td><p>&nbsp;</p>
