...
class Validator 
{
	...
	protected $items;
	protected $rules = array('required', 'minlength' , 'maxlength', 'email', 'alnum', 'match');
	public $messages = array(
						'required' => 'The :field field is required',
						'minlength' => 'The :field field is be a minimum of :satisfier length',
						'maxlength' => 'The :field field is be a maximum of :satisfier length',
			...
			if(in_array($rule, $this->rules))
			{
				if(!call_user_func_array(array($this, $rule), array($field, $item['value'], $satisfier)))
				{
					$this->errorHandler->addError(
								str_replace(array(':field', ':satisfier'), array($field, $satisfier), $this->messages[$rule]), 
								$field
							);
				}
